...
import { Star, Share2, Pencil, ThumbsUp, MapPin, Phone, MessageSquare, MessageCircle, Check,Bookmark } from "lucide-react";
...
export function BusinessCard({ business }: { business: Business }) {
  ...
  const randomizeLastDigit = (id: string) => {
        ...
        >
          <Star
            className={`w-6 h-6 transition-colors duration-300 ${
              idx <= hoverIndex ? "fill-yellow-400 text-yellow-400" : "fill-gray-400 text-gray-400"
            }`}
          />
        </div>
      ))}
