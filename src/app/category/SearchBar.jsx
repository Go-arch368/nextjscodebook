...
const SearchBar = () => {
  ...
  const fetchResults = useCallback(
    debounce(async (query, pin) => {
      ...
      try {
        ...
        console.log('Fetching search results for:', `/api/search?${queryParams.toString()}`);
  ...
  const handleSearch = (e) => {
    ...
    if (searchQuery) {
      ...
      console.log('Navigating to search:', `/category?${queryParams.toString()}`);
  ...
  const handleSelect = (item) => {
    ...
    console.log('Navigating to selected item:', `/category?${queryParams.toString()}`);
    router.push(`/category?${queryParams.toString()}`);
    setSearchQuery('');
    setIsSearchOpen(false);
  };
